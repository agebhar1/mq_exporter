...
FROM registry.access.redhat.com/ubi8/ubi-minimal:8.10-1018 AS builder
...
RUN microdnf install tar gzip findutils gcc make git \
    ...
    && curl -L https://go.dev/dl/go1.22.6.linux-amd64.tar.gz | tar xzf - -C /opt
...
LABEL \
    ...
    org.opencontainers.image.base.name="registry.access.redhat.com/ubi8/ubi-minimal:8.10-1018"
