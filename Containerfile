...
FROM registry.access.redhat.com/ubi8/ubi-minimal:8.8-1014 AS builder
...
RUN microdnf install tar gzip findutils gcc make git \
    ...
    && curl -L https://go.dev/dl/go1.20.7.linux-amd64.tar.gz | tar xzf - -C /opt
...
LABEL \
    ...
    org.opencontainers.image.base.name="registry.access.redhat.com/ubi8/ubi-minimal:8.8-1014"
