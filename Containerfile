...
FROM registry.access.redhat.com/ubi8/ubi-minimal:8.10-1778576163 AS builder
...
RUN microdnf install tar gzip findutils gcc make git \
    ...
    && curl -L https://go.dev/dl/go1.26.3.linux-amd64.tar.gz | tar xzf - -C /opt

COPY . ./
RUN PATH=$PATH:/opt/go/bin make

FROM registry.access.redhat.com/ubi8/ubi-minimal:8.10-1778576163
...
LABEL \
    ...
    org.opencontainers.image.base.name="registry.access.redhat.com/ubi8/ubi-minimal:8.10-1778576163"
